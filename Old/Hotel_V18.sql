...
CREATE FUNCTION one_manager() RETURNS trigger as $$
	...
	BEGIN
		SELECT COUNT(roles_positions) 
			...
			) as x WHERE roles_positions = 'manager'
			GROUP BY roles_positions into result;
			
			SELECT employee_hotelID FROM employee natural join  WHERE NEW.SSN_SIN;
...
CREATE TRIGGER one_manager before insert on employee FOR EACH ROW

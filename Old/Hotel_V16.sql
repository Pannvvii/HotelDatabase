...
CREATE TABLE customer (
	SSN_SIN int CHECK (699999999 <= SSN_SIN AND  1000000000 > SSN_SIN),
...
CREATE TABLE rents (
	...
	CONSTRAINT fk_hotelID
		FOREIGN KEY (hotelID)
			REFERENCES hotel(hotelID) ON DELETE CASCADE,
	PRIMARY KEY(SSN_SIN,hotelID)
);

CREATE TABLE manager (
	hotelID int,
	manager_SSN int CHECK (699999999 <= manager_SSN AND  1000000000 > manager_SSN),
	employee_SSN int CHECK (699999999 <= employee_SSN AND  1000000000 > employee_SSN) UNIQUE,
	CONSTRAINT fk_hotelID
		FOREIGN KEY (hotelID)
			REFERENCES hotel(hotelID) ON DELETE CASCADE,
...
DROP FUNCTION IF EXISTS add_rents(hotelID int, SSN_SIN int);
CREATE FUNCTION add_rents(hotelID int, SSN_SIN int) RETURNS void LANGUAGE plpgsql as 
$$
declare
	BEGIN
		INSERT INTO rents(hotelID, SSN_SIN) VALUES(hotelID,SSN_SIN);
	END;
$$;
...
DROP INDEX IF EXISTS hotel_chainID_index;
DROP INDEX IF EXISTS hotel_hotelID;
...
CREATE INDEX hotel_chainID_index ON hotel(hotel_chainID);
CREATE INDEX hotel_hotelID ON hotel(hotelID);
...
DROP INDEX IF EXISTS chain_name_uni;
DROP INDEX IF EXISTS address_chain_uni;
DROP INDEX IF EXISTS contact_email_uni;
DROP INDEX IF EXISTS address_hotel_uni;

CREATE INDEX chain_name_uni ON hotel_chain(chain_name);
CREATE INDEX address_chain_uni ON hotel_chain(address);
CREATE INDEX contact_email_uni ON hotel(contact_email);
CREATE INDEX address_hotel_uni ON hotel(address);
...
(42,5,1,'GoneOneNight3@gmail.com','6 Kitone St','Quebec'),(43,5,3,'GoneOneNight4@DontSleepAsWell8.com','87 Dia St','London'),
(44,5,2,'GoneOneNight5@gmail.com','345 Filla St','Quebec'),(45,5,5,'GoneOneNight6@DontSleepAsWell8.com','654 Pila St','London'),
...
(929238916,'Wings','Johnson','Generic St','servant');
